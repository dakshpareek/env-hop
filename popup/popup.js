...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  // Find project for current URL
  const project = await findProjectForUrl(tab.url, data);

  if (project && !project.isNewEnvironment) {
    // Site is configured - show simple message
    showConfigured();
  } else if (project && project.isNewEnvironment) {
    // Related project found - show smart detection
    showRelatedProject(project, tab.url);
  } else {
    // New site - show add form
    showAddNew(tab.url);
  }
});
...
async function saveNewEnvironment(url, envType) {
  ...
  // Update badge and close popup
  await updateBadge(currentTab.id, envType);
  window.close();
}
